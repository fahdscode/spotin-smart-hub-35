-- Fix the toggle_client_checkin_status function to handle failed barcode lookups properly
...
EXCEPTION WHEN OTHERS THEN
    -- Log the error with debug info - but only if we have a valid scanned_by_user_id or client_id
    IF p_scanned_by_user_id IS NOT NULL OR client_record.id IS NOT NULL THEN
        INSERT INTO public.check_in_logs (client_id, action, scanned_barcode, scanned_by_user_id, notes)
        VALUES (client_record.id, 'system_error', trimmed_barcode, p_scanned_by_user_id, 
