...
const signupSchema = z.object({
  ...
  howDidYouFindUs: z.string().min(2, "Please tell us how you found us").max(200, "Response must be less than 200 characters"),
  gender: z.string().min(1, "Please select your gender"),
  birthday: z.string().min(1, "Please enter your birthday").refine(date => {
...
const ClientSignup = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      } = await supabase.rpc('test_client_registration', {
      ...
      <div className="w-full max-w-2xl space-y-8">
        ...
        <div className="grid grid-cols-3 gap-4 text-center">
          ...
          <div className="flex flex-col items-center space-y-2">
            ...
            <span className="text-sm text-gray-600">و شاركنا في مجتمعنا</span>
          </div>
          <div className="flex flex-col items-center space-y-2">
        ...
        <Card className="border-0 shadow-xl bg-white/80 backdrop-blur-sm">
          ...
          <CardContent>
              ...
              <div className="grid grid-cols-1 gap-4">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <Input id="firstName" name="firstName" type="text" placeholder="Enter your first name" value={formData.firstName} onChange={handleInputChange} className={errors.firstName ? "border-destructive" : ""} />
                    {errors.firstName && <p className="text-sm text-destructive mt-1">{errors.firstName}</p>}
                  </div>
                  
                  <div>
                    <Label htmlFor="lastName">Last Name</Label>
                    <Input id="lastName" name="lastName" type="text" placeholder="Enter your last name" value={formData.lastName} onChange={handleInputChange} className={errors.lastName ? "border-destructive" : ""} />
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <Input id="birthday" name="birthday" type="date" value={formData.birthday} onChange={handleInputChange} className={errors.birthday ? "border-destructive" : ""} max={new Date(Date.now() - 16 * 365 * 24 * 60 * 60 * 1000).toISOString().split('T')[0]} />
                ...
                <div>
                  ...
                  <Input id="phone" name="phone" type="tel" placeholder="e.g., 1234567890" value={formData.phone} onChange={e => {
                ...
                <div>
                  ...
                  <Input id="email" name="email" type="email" placeholder="Enter your email address" value={formData.email} onChange={handleInputChange} className={errors.email ? "border-destructive" : ""} />
                  {errors.email && <p className="text-sm text-destructive mt-1">{errors.email}</p>}
                </div>

                <div>
                  <Label htmlFor="password">Password</Label>
                  <Input id="password" name="password" type="password" placeholder="Min 8 chars, uppercase, lowercase, number" value={formData.password} onChange={handleInputChange} className={errors.password ? "border-destructive" : ""} />
                  {errors.password && <p className="text-sm text-destructive mt-1">{errors.password}</p>}
                </div>

                <div>
                  <Label htmlFor="confirmPassword">Confirm Password</Label>
                  <Input id="confirmPassword" name="confirmPassword" type="password" placeholder="Confirm your password" value={formData.confirmPassword} onChange={handleInputChange} className={errors.confirmPassword ? "border-destructive" : ""} />
                  {errors.confirmPassword && <p className="text-sm text-destructive mt-1">{errors.confirmPassword}</p>}
                </div>

                <div>
                  <Label htmlFor="jobTitle">Job Title / Profession (optional)</Label>
                  <Input id="jobTitle" name="jobTitle" type="text" placeholder="e.g., Software Engineer, Marketing Manager, Freelancer" value={formData.jobTitle} onChange={handleInputChange} className={errors.jobTitle ? "border-destructive" : ""} />
                  {errors.jobTitle && <p className="text-sm text-destructive mt-1">{errors.jobTitle}</p>}
                  
                  <div className="flex items-center space-x-2 mt-2">
                    <input type="checkbox" id="isStudent" name="isStudent" className="rounded border-gray-300" />
                ...
                <div>
                  ...
                  {errors.howDidYouFindUs && <p className="text-sm text-destructive mt-1">{errors.howDidYouFindUs}</p>}
                </div>
              </div>
            ...
            <div className="mt-4 text-center space-y-2">
              <a href="/client-login" className="text-sm text-green-600 hover:text-green-700 transition-colors font-medium">
