  ...
  const handleToggleClientStatus = async (clientId: string, currentStatus: boolean) => {
    try {
      const { data, error } = await supabase.rpc('toggle_client_checkin_status', {
        p_barcode: clients.find(c => c.id === clientId)?.barcode || '',
        p_scanned_by_user_id: null
      });

      if (error) throw error;

      const result = data as { success: boolean; error?: string; action?: string; client?: any };
      
      if (!result?.success) {
        throw new Error(result?.error || 'Failed to toggle client status');
      }

      toast.success(`Client ${result.action === 'checked_in' ? 'checked in' : 'checked out'} successfully`);
      await fetchClients();
    } catch (error: any) {
